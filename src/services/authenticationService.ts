...
export async function register(user: RegistrationUserDto): Promise<void> {
  const response = await fetchData(
    `http://localhost:8080/buzzbyte/auth/register`,
    {
      method: "POST",
      headers: {
...
export async function enableUser(token: string): Promise<void> {
  const response = await fetchData(
    `http://localhost:8080/buzzbyte/auth/enable/${token}`,
    {
      method: "PUT",
    }
  );
...
export async function login(loginRequest: LoginRequest): Promise<void> {
  ...
  const response = await fetchData(
    `http://localhost:8080/buzzbyte/auth/login`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export async function requestPasswordReset(email: string): Promise<void> {
  const response = await fetchData(
    "http://localhost:8080/buzzbyte/auth/reset-password",
    {
      method: "POST",
      headers: {
...
export async function getRandomPassword(token: string): Promise<String | void> {
  try {
    const response = await fetchData(
      `http://localhost:8080/buzzbyte/auth/reset-password-email/${token}`,
      {
        method: "POST",
      }
    );
...
export async function changePassword(
  ...
  const response = await fetchWithAuth(
    "http://localhost:8080/buzzbyte/users/password",
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
...
export async function modifyUser(
  ...
  const response = await fetchWithAuth(
    `http://localhost:8080/buzzbyte/users/${userId}/modify`,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
